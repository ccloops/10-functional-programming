...
(function(module) {
  ...
  Article.numWordsAll = () => {
    return Article.all.map(articleObject => articleObject.body.split(' ').length).reduce((a,b) => a + b);
  ...
  Article.allAuthors = () => {
    return Article.all.map(articleObject => articleObject.author).reduce((acc, author) => {
  ...
  Article.numWordsByAuthor = () => {
    ...
    // The first property should be pretty straightforward, but you will need to chain some combination of .filter(), .map(), and .reduce() to get the value for the second property.
      let eachAuthor = Article.all.filter(articleObject => articleObject.author === author).map(articleObject => articleObject.body.split(' ').length).reduce((a,b) => a + b);
      return {
        authorName: author,
        numberWords: eachAuthor
      }
    })
  };
